...
BOOL GpuUsage::Init()
{
    ...
    std::wstring libFileName = QString("nvapi.dll").toStdWString();
    m_hNvApiDll = LoadLibrary(libFileName.c_str());
    if (m_hNvApiDll)
    {
...
BOOL GpuUsage::GetDisplayInfo(const int nCardIndex, GPU_DISPLAY_INFO *pDisplayInfo)
{
    BOOL bResult = FALSE;
...
int GpuUsage::GetDisplayCardCount()
{
    return m_nDisplayCardCount;
}

int GpuUsage::EnumDisplayCards()
{
    NvStatus nvResult;
...
BOOL GpuUsage::GetGpuHandles(const NvDisplayHandle nvDisplayHandle, DISPLAY_CARD_INFO *pCardInfo)
{
    BOOL bResult = FALSE;
...
BOOL GpuUsage::GetDisplayCardGpuUsages(const NvDisplayHandle nvDisplayHandle, DISPLAY_CARD_INFO *pCardInfo)
{
    Q_UNUSED(nvDisplayHandle);
    BOOL bResult = FALSE;
...
BOOL GpuUsage::GetDisplayCardMemoryInfo(const NvDisplayHandle nvDisplayHandle, DISPLAY_CARD_INFO *pCardInfo)
{
    BOOL bResult = FALSE;
